// Modified version main function of tagRFID_5.c
...
int spi_transfer(uint8_t* tx_data, uint8_t* rx_data, int len) {
    ...
    // 1개의 SPI 메시지 전송할 때, 오류 발생 시 error message 호출
...
void readUID(uint8_t* uid) {
    ...
    if (command >= 0x00 && command<=0xFF) {
        ...
        // 슬레이브 장치는 각각의 고유한 SS 핀을 가짐, master-slave 통신 위해서 ss핀을 LOW/HIGH로 설정 
        ...
        print_hex_message(uid, sizeof(uid));
...
// UID 정보 UART로 write
void write_uid_uart(uint8_t* uid, int len) {
...
// UART information -> PC
void pc_receive_uid(int uart_fd, uint8_t* uid, size_t uid_size) {
    ssize_t bytes_read = read(uart_fd, uid, uid_size);
    if (bytes_read > 0) {
        printf("Receive UID from PC : ");
...
// UID 정보 읽기
    while (1) {
        ...
        usleep(500000);
        // UID 정보 UART로 write
        write_uid_uart(uart_fd, sizeof(uart_fd));
/*
        // first, find to detected UID info addr
        // UART 데이터 송수신(to PC)
        uint8_t tx_data[] = {detected UID info addr};
        uint8_t rx_data[3];

        print_hex_message(uid, length(uid));

        if (uart_transfer(tx_data, rx_data, sizeof(tx_data)) != 0) {
            printf("UART transfer failed \n");
        }
                or
*/
        pc_receive_uid(uart_fd, uid, uid_size);
    }
